...
## 기능 5 — 위성 (첨부파일 표시, 추가 요청 2026-07-07)

- 첨부파일이 있는 노트의 행성 주위에 작은 위성이 공전한다.
- 판정: 프론트에서 `note.content`에 포함된 `/attachments/file/` 출현 횟수
  (백엔드 변경 없음). 위성 수는 첨부 수만큼, 최대 3개로 상한.
- 렌더링: 행성 반지름의 ~0.22배 회색 구체, 행성 중심 기준 반지름
  행성×1.8~2.6의 작은 궤도를 프레임마다 공전 (행성 mesh의 자식으로 붙여
  공전 궤도를 단순화).
- 행성 툴팁에 첨부 수 표시 (예: "제목 · 📎2").
